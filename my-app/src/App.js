...
function Header(){
  return <header>
  <h1 class="head"><a href="/">고잉 세븐틴 안내서</a></h1>
</header>
}
function Main(props){
  return <main>
    <h3 class="mainhead">{props.title}</h3>
    <p class="maintext1">{props.body}</p>
  </main>
}
function Nav(props){
  ...
  if(props.going){
    for(let i = 0; i < props.going.length; i++){
      let t = props.going[i];
      goingList.push(<a id={t.id} href={'/read/'+t.id} ><li class="navtext2" onClick={(event)=>{
  ...
  return (<nav>
    <ul>
        <a href="/" ><li class="navtext1 navch1">소개</li></a>
